...
export const projects = [
  ...
  },
  {
    ...
    "runtime": "static-artifact",
    "version": "1.6.0",
    "summary": "몬스터가 직원인 회사에 월드 매니저로 입사해, 몬스터를 진화시키고 던전을 이어 모험가(NPC)들이 레벨 1부터 70까지 즐겁게 올라가는 월드를 만드는 저관여 월드 매니지먼트. 메인 화면 \"월드 길\"은 가로축이 레벨이고, 던전은 자기 적정 구간 너비의 발판, 발판이 없는 땅이 빈틈이다. 핵심 결정은 \"지금 진화시킬까?\" — 진화하면 위가 열리지만 아래가 빈다. 5장(헤네시스→슬리피우드) 전부와 주니어 발록 엔딩, 완전 클리어(던전 19곳 ★3·도감 52)까지 돈다. 규칙 v1.2(선택 점검)로 어떤 선택도 월드를 멈추지 않게 했고, 규칙 v1.3(플레이 리뷰 뒤)으로 첫 세션 약 3분 안에 승진 발령·1장 결재 도장·새 지역까지 한 바퀴를 돌게 했고, 규칙 v1.4로 배속 없이 파티 도착·구간 개방·초반 사냥터로 첫 40분 동안 사건이 최장 20초마다(결정은 약 1분에 한 번) 일어나게 했다. 규칙 v1.5로 헤네시스·엘리니아 계열을 지역당 4종으로 늘려, 줄이 서면 자리 확장 대신 레벨이 다른 계열로 던전을 하나 더 열어 줄을 나누는 선택지가 생겼다. 규칙 v1.6으로 2장부터 사냥이 쌓이면 던전 앞에 드랍 상자가 떨어져, 줄·빈틈에 맞는 계열 채용권과 무료 이벤트권 가운데 하나를 고른다(월드에 하나, 쥔 권이 3장이면 쉰다). 3~5장에는 결재 막대 눈금 보상, 사냥이 쌓이면 나오는 엘리트, 장마다 한 번 찾아오는 필드 보스가 봉우리를 만든다. ▲ 진화 배지는 지금 해도 되는 것만 최대 3개. 40판 모두 D23~38에 엔딩(표준 D30), 멈춘 월드 0.",
    "tags": [
      "game",
      "idle",
      "management",
    ...
    "updatedAt": "2026-09-26T02:41:07+00:00"
  },
  {
    "id": "retro-bowling",
    "name": "Retro Bowling",
