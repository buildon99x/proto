...
export const projects = [
  ...
  },
  {
    ...
    "runtime": "static-artifact",
    "version": "0.7.1",
    "summary": "지구 출토(Findspot: Earth) — 떠나지 않은 사람들이 묻어 둔 박물관을 파내는 도굴 팀이, 257년 비어 있던 카드의 다음 줄에 누구의 이름을 쓸지 정한다. v0.7.0은 게임 한 편을 IP로 키운 릴리스다: 세계관 바이블(게임 화면 층과 바이블 층), 크루 다섯의 아크와 다른 형식 피치 넷, 크루 5인 도트·캐릭터 시트·키 비주얼·로고·장소 컨셉, 한 페이지 쇼케이스(/runs/relic-king/ip/). v0.7.1에서 그 세계가 게임 화면에 닿았다 — 발굴 탭의 팀 등록증(세계 다섯 문장·크루 다섯·찾는 한 점), 상자마다 붙는 기록 카드(위는 실재 소장처와 마지막 확인 2094년 3월, 점선 아래 다음 확인 칸에 우리 팀·박물관·산 사람의 이름), 화면마다 크루 한 명의 한 줄. 규칙·상수·세이브는 그대로다. 실존 유물을 발굴·감정·소장해 3축 순위(자산·도감·명성) 종합 1위에 오르는 방치형. 유물의 현실 현존 개체 수가 그대로 게임 공급량이고, 세상에 하나뿐인 유물은 제보 레이스에서 먼저 도달한 쪽이 영구히 가진다. v0.6에서 첫 세션의 밀도를 다시 설계했다 — 첫 제보가 14분 30초에서 20초로, 첫 레이스 결과가 15분에서 1분 20초로, 첫 유일(T4) 조우가 '한 시간 안에 못 만남'에서 4분 35초로 당겨졌고, 첫 1분에 겪는 사건이 3종에서 5종, 첫 10분이 4종에서 12종이 됐다. 반복 대 의미는 7.14:1에서 1.30:1이다. 그 대가로 전체가 짧아졌다 — 엔딩이 140시간 36분에서 16시간 25분이 됐다(8.6배 압축). 제보 배너에는 반응 유예 30초가 붙어 창이 설계값(60~150초) 안으로 돌아왔고, 라이벌 적중률이 플레이어와 같은 28%가 되어 아무것도 안 누르면 반반, 집중 굴착을 누르면 60%가 된다. 도감이 차면 선점 승산이 오르고(안목), 소장고 정원 초과는 자동으로 처리된다. v0.5에서 플레이어끼리 겨룰 수 있게 됐다 — 내 상태를 200자 남짓 코드(기록패)로 구워 건네면 받은 쪽 세계에서 계속 자라는 상대가 된다(서버·런타임 네트워크 0). 지도는 Natural Earth 1:110m 육지를 빌드타임에 구운 실제 해안선이고, 12거점은 실존 도시 좌표에 찍힌다(경주·룩소르·폼페이·이스탄불…). 32×32 도트 아이콘이 shape 10종 안에서 실루엣 60종으로 갈라지고, 유일 12종은 손으로 찍은 전용 도트를 쓴다. 유물 2000종(검증 1,902종)은 메트로폴리탄 미술관 오픈액세스(CC0) 메타데이터로 만든 종 1,720개를 손으로 쓴 280종 위에 얹은 것이다. 5탭(발굴·소장고·시설·시장·도감) UI로 돌아가고, 화폐는 달러(흔한 유물 한 점 ≈ $1만)다. 소장고에서는 여러 종을 골라 한 번에 매각·경매 등록할 수 있다(v0.5.2). 클릭 0회 방치만으로 엔딩에 도달한다.",
    "tags": [
      "game",
      "idle",
      "incremental",
    ...
    "docs": {
      ...
      "prompts": "prompts/ip-world-narrative-visual.md",
