...
//    단계마다 걸린 시간(월드 시각 기준 실제 초), 입력 수, 화면 밀도를 적는다.
...
const MEASURE = `(() => {
  ...
  return {
    clock: Math.round(A.w.t * 60), step: T.st.done ? null : (T.cur() ? T.cur().id : null),
    ...
    badges: { gap: q('.gapb').length, evolve: q('.evb').length, busy: q('.pill.busy').length },
...
try {
  ...
  await until(s => s.step === "event", "event");
  await mark("fixed", "뚫렸다!! — 오렌이 이벤트를 권한다");
  await click('.plat[data-plat="h1"] .body');
  await click('[data-evt="exp"]');
  await mark("event", "경험치 2배 (첫 번 무료)");
  await click("#dBack");
  await until(s => s.step === "evolve", "evolve", 120);
  await pump(1);
  await mark("evolve-badge", "보라 ▲ — 고참 달팽이 진화 가능");
  await click(".evb");
  await mark("evolve-sheet", "진화 시트 — 사원증 두 장, 결과 한 줄, 버튼");
  await click("#sheet [data-go]"); await sleep(1500);
  await mark("evolve-cut", "진화 컷 — 파란 달팽이");
  ...
  await mark("tut-done", "튜토리얼 끝 — 퇴근해도 월드는 돈다");
  // 퇴근 → 다음 날 아침 (10시간)
  ...
  // ── ② 시연 장면: 후반 화면 밀도 ──
  const DEMOS = [["gap", "1장 첫 빈틈"], ["hire", "채용 시트"], ["dungeon", "던전 현장"], ["evolve", "진화 시트"], ["report", "출근 리포트"], ["approval", "결재함"], ["ch2", "2장 엘리니아"], ["promote", "3장 승진 발령"], ["grow", "키워서 잇기"], ["late", "4장 후반 월드"], ["ch5", "5장 슬리피우드"], ["codex", "도감"], ["ending", "엔딩"], ["fullclear", "완전 클리어"]];
