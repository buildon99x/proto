...
const STEPS: Step[] = [
  { id: 'arrive', line: '모험가님들이 자기 레벨 던전을 찾아 들어와요!!', spot: () => null, done: s => s.age > 7 && M.happyCount(w()) >= 2 },
  { id: 'watch', line: '“헤네시스 들판” 발판을 눌러서 구경해봐요!!', spot: () => (A.ui.mode === 'world' ? plat('h1') : null), done: s => A.ui.mode === 'dungeon' || !!s.flags.levelup },
  { id: 'levelup', line: '즐거운 모험가님은 빛기둥과 함께 레벨업해요!!', spot: () => null, done: s => (!!s.flags.levelup && s.age > 5) || s.age > 30 || A.ui.mode === 'world' },
  { id: 'back', line: '월드로 돌아가면 자란 모험가님이 위 던전으로 옮겨가는 게 보여요!!', spot: () => (A.ui.mode === 'dungeon' ? '#dBack' : null), done: () => A.ui.mode === 'world' },
  {
    id: 'grow', line: s => ['가로 = 레벨이에요!! 모험가님은 레벨업하면 오른쪽으로 걸어가요!!', '발판 폭 = 그 던전에 맞는 레벨이에요!!'][Math.floor(s.age / 7) % 2],
    spot: () => null, done: () => stuck().length > 0 || !!mush(),
  },
  {
    id: 'gap', line: () => `Lv ${stuck()[0] ? stuck()[0].lv : 8} 모험가님이 갈 데가 없대요!! 빨간 !를 눌러요!!`,
    spot: () => (A.ui.mode === 'world' && !A.ui.sheet ? '.gapb' : A.ui.mode === 'dungeon' ? '#dBack' : null), done: () => A.ui.sheet === 'hire' || !!mush(),
  },
  { id: 'hire', line: '입사 선물 채용권으로 주황버섯(Lv 8)을 뽑아요!!', spot: () => (A.ui.sheet === 'hire' ? '[data-hire="mush"]' : '#bHire'), done: () => !!mush() },
  {
  ...
  { id: 'fixed', line: () => (M.gapSegments(w()).some(g => g[0] <= 13) ? '아직 끊겨 있어요!! 주황버섯을 “사냥터”로 옮겨봐요!!' : '뚫렸다!! 모험가님들이 다시 올라가요!!'), spot: () => null, done: s => s.age > 6 && !M.gapSegments(w()).some(g => g[0] <= 13) },
  // v1.4: 길이 이어지면 헤네시스 다음 구간(Lv 11–15)이 열린다. 퇴근으로 열리지만 대본은 6초 넘게 기다리게 하지 않는다
  {
    id: 'zone', line: () => (M.zoneLeft(w()) ? '퇴근이 쌓이면 졸업 문 너머 🔒 구간이 열려요!!' : `헤네시스 Lv ${zoneFrom()}–${M.roadEnd(w())} 구간이 열려 졸업 문이 오른쪽으로 갔어요!!`),
  ...
  {
    id: 'vetwait', line: 'Lv 14–15는 채용으로 안 닿아요!! 고참 달팽이 근속이 거의 찼으니 진화시켜요!!', spot: () => (A.ui.mode === 'dungeon' ? '#dBack' : null),
  ...
  {
    id: 'promote', line: () => (A.ui.sheet === 'evolve' ? '[▲ 승진 발령]을 눌러요!! 진화하면서 버섯 언덕을 열어 한 번에 이어요!!' : '고참 달팽이의 보라색 ▲를 눌러요!!'),
  ...
  {
    id: 'promoted', line: () => {
      if (road()) return 'Lv 1–15가 이어졌어요!! 승진 발령 = 진화 + 맞는 던전으로 옮기기예요!!';
      if ($('#toast button')) return '5초 안에 되돌리고 승진 발령을 골라봐요!!';
      const mv = fix();
      return mv ? `${josa(M.monName(mv.mon), '을', '를')} “${plotName(mv.to)}”로 끌어 놓으면 이어져요!!` : '빨간 !를 눌러 잇는 법을 봐요!!';
    },
    spot: () => {
      if (road()) return null;
  ...
  {
    id: 'event', line: () => (A.ui.mode === 'dungeon' && A.dv.id === 'h1' ? '첫 번은 공짜인 경험치 2배를 눌러요!!' : '이벤트를 걸어볼까요?! 들판을 눌러요!!'),
    spot: () => (A.ui.mode === 'world' ? plat('h1') : A.dv.id === 'h1' ? '[data-evt="exp"]' : '#dBack'), done: () => M.activeEvents(w()) > 0 || w().tickets.event === 0,
  },
  { id: 'burst', line: '두 배로 빨리 자라요!! 대신 다음 던전이 붐빌 수 있어요!!', spot: () => null, done: s => s.age > 8 },
  ...
  {
    id: 'joy', line: () => `결재 막대 ${Math.floor(Math.min(100, (100 * w().cjoy) / Math.max(1, M.approvalConds(w()).joyGoal)))}%!! 😊가 많을수록 빨리 차고, 퇴근해도 계속 차요!!`,
  ...
  {
    id: 'recruit', line: () => (A.ui.sheet === 'recruit' ? '모객권으로 [신규 모객]을 걸어요!!' : '엘리니아가 열렸으니 손님을 더 불러요!! 📣 모객을 눌러요!!'),
  ...
  {
    id: 'bossTease', line: '막대 가운데 눈금까지 차면 필드 보스가 찾아와요!!',
    spot: () => (A.ui.modal || !ch2() ? null : '#docw'), done: s => !ch2() || s.age > 9,
  },
  { id: 'bye', line: '퇴근해도 월드는 돌아가요!! 다음 출근 때 막대를 봐요!!', spot: () => null, done: s => s.age > 14 },
];
...
export const T = {
  ...
  /** 튜토리얼이 모객을 가르치기 전에는 📣 모객 버튼을 권하지 않는다 (한 번에 하나만, v1.7) */
  hideRecruit: () => T.active() && T.st.step < STEPS.findIndex(s => s.id === 'recruit'),
