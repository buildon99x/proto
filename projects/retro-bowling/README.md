...
## 조작
- 키보드: `← →` 조준 · `Space`/`Enter` 파워→스핀→투구 · `M` 음소거 · `O` 오일 패턴
- 모바일: 하단 `◀ ● ▶` 버튼, 캔버스 탭 = 액션, `OIL` 버튼으로 상급 모드 토글
...
## 오일 패턴 (상급 모드)
`O` 키(또는 `OIL` 버튼)로 켜는 상급 레이어. 레인마다 오일 분포가 달라 볼의 훅 양이
바뀌고, 게임마다 새로 뽑힌다. HUD의 `OIL DRY/MEDIUM/OILY` 표시를 읽고 매 게임 라인을
조정해야 한다. 추천 조준·궤도 프리뷰가 오일에 맞춰 보정되어 겨냥은 안내하지만, 포켓
진입각과 스트라이크율은 오일마다 달라 정복은 여전히 스킬. 기본은 꺼짐(균형 유지).
