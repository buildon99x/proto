...
- [ ] **첫 실행 안내를 "읽었다"가 아니라 "해 봤다"로 기억하는가** — 읽고 넘긴 화면이었다면 brief 의 조항을 어긴 것이다
- [ ] **한 신에서 10초 안에 파악되는가** — 보이는 글자가 45자(초당 5~7자로 6.4~9.0초)이고 스크롤이 없다. 글자를 더 얹을 때마다 이 수치를 다시 재야 한다
- [ ] **안내에서 첫 판까지 끊김 없이 이어지는가** — 마지막 누름과 출발 사이에 "시작하기"를 한 번 더 누른 느낌이 드는지
- [ ] **두 번째 실행에 안내가 없다는 것을 불편해하지 않는가** — 다시 보기가 홈 푸터에 있다는 것을 찾을 수 있는가
- [ ] 안내를 받은 뒤 첫 10초 안에 버튼의 의미를 파악하는가 (0.7.0 이전에는 "조작 설명 없이"였다)
