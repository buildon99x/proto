...
- **게이트의 24%가 교환이 아니다.** 축이 상한에 닿으면 `applyTrade` 의 한쪽만 먹혀 순손해·순이득·무효가 생긴다(±2 기준 각 9%·9%·6%). `canOffer` 가 이를 막으려고 존재하지만 호출되지 않는다. 게이트 제안을 **직전 게이트를 지날 때 현재 빌드로 정하는** 것이 답이고, 그러면 스테이지 시드를 다시 큐레이션해야 한다.
