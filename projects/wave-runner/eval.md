...
| **벽 도료 사양** | 6종 전부 대비 바닥 통과(엣지 최저 9.51 · 장애물 5.06 · 벽/배경 1.137~1.321) · 도료끼리 ΔE 6.0~49.3 · 해금은 진행도에서 파생 |
| **여백이 통로를 침범하는가** | 172장면 1280×720 DPR2 픽셀 대조, 도료 6종을 돌려 가며 — 통로 안 **0 픽셀**, 근접대 **0 픽셀**(최소 여유 7.88월드) |
...
### 벽 도료 — 자동 검증이 증명하지 못하는 것

대비 바닥은 **"읽을 수 있다"를 보장할 뿐 "똑같이 쉽다"를 보장하지 않는다.** 가장 밝은 도료(비취)는 벽/배경 대비가 1.137 → 1.321 로 올라가므로 도형-바탕 관계가 조금 달라진다. 질감 A/B 에 비취 조건을 하나 더 붙여 최초 클리어까지의 시도 수를 본다 — 기본 도료 대비 ±10% 를 벗어나면 벽/배경 상한을 1.35 → 1.25 로 내린다(도료를 뺄 필요는 없다).

채택률(도료를 해금한 사람 중 기본이 아닌 것을 고른 비율)이 40% 미만이면 **종류를 늘리지 말고 해금 조건을 앞당긴다.** 부족한 것은 다양성이 아니라 도달이다.
...
- [ ] **도료를 바꾸면 눈에 띄게 달라지는가** — 그리고 달라진 벽에서도 통로가 똑같이 읽히는가
- [ ] **도료가 보상으로 느껴지는가** — 힘을 주지 않는다는 것이 실망이 아니라 안심으로 읽히는가
