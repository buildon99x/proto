...
import { LOCAL_DIR, credentials, diagnose, nextSteps, prefixFor } from "./blob";
...
async function main() {
  ...
  if (cred.kind === "none" || cred.kind === "need-store-id") {
    bad(cred.note);
    console.log("");
    for (const line of diagnose()) info(line);
    console.log("");
    for (const line of nextSteps(cred.kind)) info(line);
    console.log("");
    localSummary();
    process.exit(1);
  }
