...
> **v0.4(G69.2)** — `SiteDef`가 `name`(나라 단위) 하나였던 걸 `city`/`country`/`anchor`
...
> **v0.4 갱신(G69.3)** — `rome`의 좌표를 40.85/14.27에서 **폼페이 유적 40.75/14.49**로
...
> 140시간 44분 → 140시간 42분으로 2분 움직였다(`eval.md` §19).
...
구현됐다**(G69.1). 아래는 설계가 아니라 지금 도는 코드의 서술이다.
...
export const WORLD_CITY_COORDS: Record<SiteId, { lat: number; lon: number }> = {
  ...
  rome: { lat: 40.75, lon: 14.49 },  // 폼페이 유적(v0.4, G69.3 — 옛 값은 나폴리였다)
