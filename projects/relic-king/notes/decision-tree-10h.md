...
2. **[집중 굴착]은 지배 전략이면서 함정이다.** 원정당 한 번만 누르면 운영 플레이의 1시간 자산이
   **5.3배**가 된다 **[실측]**. 누를지 말지 고민할 이유가 없다. 그런데 뜰 때마다 누르면 원정비
   ...
   파산한다(§6 버그 1).
3. ~~자동 매각 기준은 숨은 결정이고, 기본값이 지배당한다.~~ **(정정, 12시드)** 3시드에서 본 "끄면
   자산 3.3배"는 유일 추첨이었다. 12시드에서는 10시간 자산 차이가 +5%로 흔들림보다 작다. 기본값을
   유지한다(`notes/autosell-default-study.md`).

> **정정(12시드 재측정, v0.6.6 병합 트리).** 위 1의 "자산 4.3배"도 3시드 값이다. 12시드로 다시 재면
> 본거지 효과는 **도감에서 안정적이다**. 10시간 도감 중앙값은 경주 유지 470종, 이스탄불 640.5종,
> 바그다드 691종(+47%)이고, 최솟값끼리도 427 대 651로 뒤집히지 않는다. 자산 배수는 약 1.5배로 줄고
> 흔들림이 크다. 델리는 오히려 경주보다 나빴다(자산 16,137M, 유일 0). 결론은 그대로다. 무게가 큰
> 결정이고, 정보가 없으면 고를 수 없다.
...
### 5.5 자동 매각 끄기 — 방치 (12시드로 정정)

3시드에서는 끈 쪽이 10시간 자산 40,032M 대 12,232M로 앞섰다. **12시드로 다시 재자 재현되지
않았다.** 중앙값은 기본 27,147M, 끔 28,520M(+5%)이고, 유일은 둘 다 1점, 순위는 둘 다 1위다.
자동 루틴 위상을 30초만 밀어도 우열이 뒤집힌다. 구조적 차이는 방치 첫 1시간 발굴력 +12%뿐이고,
원인은 중복 배치 정리 문턱(20점)과의 간섭이다. 상세·재현은 `notes/autosell-default-study.md` **[실측]**.
**결론: 기본값을 유지한다.** 이 절의 교훈은 따로 있다. 10시간 자산은 유일 한 점에 크게 흔들리므로,
결정의 무게를 잴 때는 자산보다 도감처럼 구조적인 지표를 먼저 봐야 한다.
...
**P7. 자동 매각 기본값을 다시 잰다.** → **12시드로 재측정했고 기본값을 유지한다**(§5.5 정정).
첫 1시간 발굴력 차이(+12%)가 문제가 되면 설정이 아니라 배치 정리 문턱의 셈법(점수가 아닌 가치 기준 등)을 고친다.
