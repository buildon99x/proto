...
σ<1 실측 플래토, 위 §0 표)에서 **잠재 화폐 창출률은 시간당 약 1.63억~5.44억 ₩**
사이다.
...
`MUSEUM_NET_INCOME_CAP = 0.30`. 참조점 기준 시간당 **최대 0.49억~1.63억 ₩**
...
| --- | --- | --- | --- |
| 유물 처분(전 채널 통합) | 발굴력에 비례하는 연속 흐름 | 1.63억~5.44억 ₩ (채널 배율 `E[LOCAL_PRICE_MULT]=1.0` 포함) | 기준 소스, 위반 없음 |
| 박물관 관람 순수입 | 관람객 기반, 캡 있음(플레이어 전체 `D` 합 기준, G24) | 최대 0.49억~1.63억 ₩ (위의 30%) | G5로 캡, §7에서 판정 |
| 시즌 시작 보너스 | 1회성 | 3만 ₩ (시즌당 1회) | 무관 |
| **보험금(조건부, G28/B6)** | **도난 발생 시에만, `THEFT_RATE_BASE(0.0014/h)`로 상한 검증됨** | **기대값은 프리미엄 수입을 넘지 않음(보험사 마진 16%, 아래 참조) — 보험금과 도난 유물 회수권은 상호배타(G28)** | **성립(조건부) — 마진이 양수인 한 화폐 복제 아님** |
...
재사용하지 않는다. 퇴직금은 G29/B7에서 급여를 매각 시점 원천징수로 바꾸며 체납·
...
**(개정 — G28/B6)** `THEFT_RATE_BASE = 0.0014`(시간당, 전시 중·T0~T3에만)로
확정했다 — 위 상한(0.00167)에 16% 여유를 둔 값이다. 보안(`S`)은 이 값 자체를
바꾸지 않고 `THEFT_INITIAL_GRACE_HOURS`(반출 직후 유예)로 **노출 시간**을 줄여
실효 발생률을 낮춘다(`spec.md` §9.4). 되사기(암시장 장물가)는 "미감정 추정가"가
아니라 **평가액** 기준(`BLACK_MARKET_STOLEN_PRICE_RATIO=0.32`)으로 통일했다 —
전시 중인 유물은 정의상 이미 감정을 마쳤으므로 "미감정가"라는 기존 기준 자체가
틀렸었다. 기준을 통일해도 `0.6(보험금) − 0.32(되사기) = 0.28`이 여전히 양수라
"보험금 받고 싸게 되사서 현물도 챙긴다"는 이중 수취가 산수로는 안 닫힌다 — 그래서
**보험금 청구와 도난 유물 회수권을 상호배타**로 만들었다: 보험금을 청구하면 그
유물은 그 즉시 영구 소실 확정 처리되고, 이후 암시장에 떠도 우선 회수권이 없다.
숫자가 아니라 상태 기계로 루프를 끊었다.
...
## 5. 정상상태 목표 — 24시간 통화 성장 초과율 (개정 — G22/A8)

**측정 대상** `M(t)`: 시각 `t`에서 플레이어와 라이벌 전원의 `funds` 합
(`player.funds + Σ rival.funds`). 유물 자산(vault)은 제외한다 — 판 돈만 통화다.
경매장 수수료는 `M`에서 소멸하는 소각분으로 잡는다(거래소 수수료 항목은 G15/A4로
삭제됐다).

**절대율 캡은 자기 수식으로 위반된다**: 소득률이 완전히 정체돼도(`M(t)=(1−s)·R·t`,
`s`는 싱크 비율) 24시간 성장률은 정확히 `24/t`다 — `t=168h`에서 이미 `24/168=
14.3% > 12%`다. 캡이 도달 불가능한 게 아니라 **정의 자체가 틀렸었다**(실측
확인: `t=48h`부터 `t=1992h`까지 어떤 시점도 절대율 12%를 안정적으로 만족하지
못한다 — 자연 감쇠항이 항상 섞여 있어서다).

**목표(재정의)**: "그 시각 자연히 발생하는 `24/t`를 얼마나 초과하는가"만 규제한다.

```
CURRENCY_GROWTH_24H_EXCESS_CAP = 0.15     // 초과 상한
CURRENCY_GROWTH_24H_EXCESS_FLOOR = -0.10  // 초과 하한(신설 — 기존엔 없어 σ<1의 "-21%"가 무조건 통과 판정을 받았다)
(M(t+24) − M(t)) / M(t) − 24/t ∈ [CURRENCY_GROWTH_24H_EXCESS_FLOOR, CURRENCY_GROWTH_24H_EXCESS_CAP]
```

`MATURE_PHASE_START_HOUR`는 168 → **48**(2일)로 낮췄다. G21(`MAX_GEAR_LEVEL=16`)
적용 후 `D(t)`가 24시간 안에 사실상 정체하므로(24h 287,954/s → 168h 318,391/s,
+10.6%) 초과율 캡이 48시간부터 의미 있게 걸린다 — 448시간까지 기다릴 필요가
없어졌다(리뷰의 원안은 상한 미적용 상태를 전제로 계산됐었다).

성숙기 이전(부트스트랩 구간)은 이 캡에서 제외한다 — 초기 성장은 온보딩의 일부이고
G11의 마일스톤 곡선이 이미 그 속도를 정의한다.

**측정 방법(개정)**: 단일 시점 24시간 차분은 지수 비용 곡선의 큰 일회성 구매
직후 표본이면 크게 요동친다(실측: 같은 궤적에서 표본 시점을 1시간 옮기는 것만으로
성장률이 −86%~+670%로 흔들렸다 — 원인은 로그가 아니라 lumpy한 구매 타이밍이다).
그래서 **7일(168시간) 이동평균으로 스무딩한 `M̄(t)`에 위 부등식을 적용**한다.
`pnpm --filter relic-king sim --hours 336`(14일)을 매각 성향 3가지 프로필로
반복 실행한다 — 공격적 매각(σ≈1에 가까움), 중립, 홀딩성향(σ가 낮음, 소장고에
오래 보유). 각 프로필에서 시간마다 `M(t)`를 로그하고 `M̄(t)`를 계산한 뒤, `t ≥ 48`인
모든 구간에서 위 부등식을 검사한다. 세 프로필 전부가 통과해야 한다 — 하나라도
넘으면 그 프로필이 지목하는 싱크(공격적 매각이면 K1·K2, 홀딩성향의 하한 위반이면
박물관 쪽 소스)를 다시 조정한다. 이 sim의 `--sell-profile` 옵션과 `M̄(t)` 로그
출력은 아직 구현되어 있지 않다 — 구현은 이 기획 문서의 범위 밖이지만, 후속 구현
작업이 반드시 추가해야 할 계측 지점으로 여기 명시한다.
...
| 도난 보험(K10) | **성립(G28로 확정)** | `THEFT_RATE_BASE=0.0014 ≤ 0.00167`로 확정해 보험사 마진 16% 확보. 보험금 청구와 회수권을 상호배타로 만들어 되사기-보험금 이중 수취 루프(review-r1.md 검산 +1.81억/건)를 구조적으로 닫았다 |
...
// G23/A9 — 심층 티어 가중 재설계(설계값, 480종 데이터 파이프라인과 함께 balance.ts에 반영)
export const LAYER_BASE_WEIGHTS_8_9 = [52, 36, 11, 0.00004, 0] as const;   // 기존 [52, 36, 11, 1, 0]
export const LAYER_BASE_WEIGHTS_10_12 = [38, 40, 18, 0.00015, 0.000025] as const; // 기존 [38, 40, 18, 3.9, 0.1]
...
export const BLACK_MARKET_STOLEN_PRICE_RATIO = 0.32; // G28/B6 — 기준을 "미감정 추정가"→"평가액"으로 통일
...
// 도난 보험(§2, G9. THEFT_RATE_BASE는 G28/B6로 신설)
...
export const THEFT_RATE_BASE = 0.0014; // 시간당, 상한 0.00167의 16% 여유. 보험금 청구 시 회수권 상호배타(G28)
