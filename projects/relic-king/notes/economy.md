...
정보 비대칭(B4)은 `notes/world-map.md` §8.5를 따른다 — `REGIONAL_PRICE_MULT_MIN/MAX`는
**정정(B5)**: 이 문서가 처음 쓰였을 때(G19/B4)는 `0.90/1.26`이었고 이 실행에서는
"재계산하지 않는다"고 못박았지만, 이후 리뷰 1회차(G26/B3)에서 개입 이득 8.3%
문제를 닫기 위해 `0.60/1.40`으로 실제로 재조정됐다(§1.1·§8·§8의 K9 인근 참조,
`notes/world-map.md` §8.2). 이 문단은 그 재조정 **이전** 시점의 기록이었다 —
현재 유효한 값은 `0.60/1.40`이다.
...
## 6.1-b 세계 총가치 재산정 (v0.3 — 데이터셋 2000종)

§6.1의 표는 `SPECIES_PER_SITE_BY_TIER = [25, 8, 4, 2, 1]` (480종 설계치)를 입력으로
썼다. 데이터셋이 실제 2,000종이 되면서 같은 공식(Σ 종수 × 종당재고 × 기준가 × 1.2,
T0 제외)을 실제 `ARTIFACTS` 배열에 적용해 다시 풀었다.

| 티어 | 종수(12거점 합) | 종당 재고 | 기준가 | 총가치 |
| --- | --- | --- | --- | --- |
| T4 | 12 | 1 | 6,000,000,000 | 864.0억 |
| T3 | 28 | 6 | 260,000,000 | 524.2억 |
| T2 | 146 | 60 | 9,000,000 | 946.1억 |
| T1 | 1,028 | 2,000 | 380,000 | 9,375.4억 |
| **합** | | | | **11,709.6억 → `ARTIFACT_WORLD_VALUE_CEILING` = 11,710억** |

T0는 786종이지만 무한 재고라 여전히 상한 계산 밖이다(§1.1의 흐름으로 다룬다).

늘어난 몫의 80%가 T1이다. 상위 두 티어가 거의 안 늘어난 건 현실이 상한이기
때문이다(`notes/decisions.md` G61) — 수집 파이프라인은 T2까지만 만든다.

**이 값은 이제 문서에만 있지 않다.** `qa_artifacts.ts`가 매번 `ARTIFACTS`에서
직접 재계산해 상수와 1% 이상 어긋나면 실패로 잡는다 — G23/A9에서 표와 실코드가
24배 어긋났던 사고를 검사로 못박았다.

`ASSET_SCORE_REF`(= 상한 × 0.15)는 375억 → **1,757억**이 됐고, 그 결과 엔딩 시점
자산 축이 1.0000(포화)에서 0.5563으로 내려와 3축 순위가 실제로 3축이 됐다.
