...
> **v0.4(`notes/decisions.md` G69.2)** — 표기가 나라("한반도·이집트·로마")에서
...
- **유물 스프라이트 32×32, 절차 생성(결정론)**:
  ...
  - **외부 이미지를 쓰지 않는 이유**: 정적 산출물은 **런타임** 네트워크 의존을 가질 수
    ...
    **빌드타임에 받아 정적 산출물로 굽는 것은 허용**이고, v0.4의 세계지도 해안선이
    그 경로로 들어왔다(`notes/decisions.md` G69.1, `notes/world-map.md` §6.1).
...
app/src/
  ...
  render/
    ...
    worldmap-raster.ts       세계지도 빌드타임 래스터 결과(생성 파일 — G8·G69.1)
  sim/
    run.ts                   헤드리스 시뮬 본체
...
인코딩은 `JSON → base64url`이다. 실측 길이 **204자**(`eval.md` §20). 압축 의존성을
넣지 않는다.

**결과값이 아니라 상태를 나른다.** 발굴력을 담고 받는 쪽에서 역산하면 반드시 틀린다
(G70.2의 실측 — 9,423/s가 4,243,517/s가 됐다). 발굴단(`teams`) 몫은 담지 않는다 —
라이벌에게는 태울 자리가 없다.
...
| | 원장에서 가져가나 | 근거 |
| --- | --- | --- |
| 기록패가 주장하는 유일 소장 | **아니다** | G70.1 — 받는 순간 내 유일이 사라지면 아무도 안 받는다 |
| 배경 발굴(`shadowTake`) | **아니다** | G70.1 — 친구를 부를수록 내 도감이 느려지면 안 된다 |
