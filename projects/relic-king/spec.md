...
수행하지 않았다(G105) — 이 절이 적용 단계의 입력이다. **v0.7.1에서 적용했다**(§18, G126).
...
### 16.7 긴급 인부 (v0.6.7, G116)

- 조건: 결판 전인 **유일** 제보, 아직 대응하지 않음, 직접 발굴이 그 거점의 그 층에 닿아 있음,
  그 자리에 현지 발굴단이 없음(`emergencyCrewOffer`). 배너·유일 알림·정책·계측이 이 함수 하나를 쓴다.
- 값: 현재 자금의 30%, 최소 $2만(`TIP_EMERGENCY_CREW_FUNDS_SHARE`·`TIP_EMERGENCY_CREW_MIN_COST`).
  모자라면 버튼 대신 "자금이 $X 모자란다. 소장품을 팔면 부를 수 있다"를 적는다.
- 효과: 대응으로 인정(`tip.focused`·`tip.crewed`), 적중 45%(`TIP_EMERGENCY_CREW_HIT_CHANCE`).
  현지 팀의 [집중 굴착](60%)보다 낮고, 아무것도 안 할 때(28%)보다 높다.
- 운영 기준선 정책은 발굴단으로 대응할 수 없을 때 긴급 인부를 부른다.

### 16.8 플레이어 경험 기준 조정 (v0.6.8, G117)

- **국보 이상은 라이벌도 집중한다.** T3·T4 제보에서 라이벌 한 명의 적중은 `TIP_RIVAL_FOCUS_HIT`(60%)로,
  플레이어의 집중과 같다(`rivalTipHit`). 진귀는 28% 그대로다.
- **"다음 확장" 카드**(`ui/ExpansionFork.tsx`). 거점이 둘, 발굴단 슬롯이 하나일 때 셋째 거점과 둘째
  발굴단 중 하나라도 살 수 있게 되면 한 번 띄운다. 두 카드는 각각 얻는 것을 한 줄로 적는다.
  발굴 탭 "내 거점"에서 다시 연다.
- **계측 게이트**: `B 결정 첫 1분`은 참고로 내리고, `C 패 첫 10분`은 참고로 두며 `C 첫 패 ≤20분`을 새 게이트로 둔다.


---

## 17. IP 층 — 「지구 출토」 (v0.7.0, `notes/decisions.md` G118~G125)

**게임의 규격은 이 절 때문에 바뀌지 않는다.** 규칙·상수·세이브·UI 문자열 변경 0이다(G124). 화면 적용은 v0.7.1에서 했다 — §18.

| 무엇 | 어디 | 게임과의 관계 |
| --- | --- | --- |
| IP 바이블 | `ip/bible.md` | `notes/world-lore.md`(§15의 화면 층)의 상위 문서. 화면에 올릴지는 §15와 G111이 정한다 |
| 비주얼 가이드 | `ip/visual-guide.md` | §4(도트 그래픽)를 큰 그림까지 늘리는 규칙. 크루 도트 16×24는 인부 스프라이트와 같은 키 |
| 그림 원본 | `ip/src/*.mjs` → `ip/art/` | 유물 도트는 `renderSpriteRGBA` 그대로(`ip/src/bake-finds.ts`) |
| 쇼케이스 | `ip/showcase/index.html` → `app/public/ip/` | 게임 번들 폴더에 복사될 뿐 게임이 읽지 않는다. 런타임 외부 요청 0 |
| 표시 이름 | `project.json` `name`, `app/index.html` `<title>` | 「지구 출토」. 슬러그 `relic-king` 유지 |

**화면에 처음 닿을 자리**: §15.5 장치 ①의 한 줄 아래에 "다음 확인: 2351년 5월 · (기록패 이름)". **v0.7.1에서
적용했다** — 기록 카드의 점선 아래 칸이다(§18.2).

---

## 18. v0.7.1 — 화면 층 적용 (`notes/decisions.md` G126~G131)

§15(화면별 한 줄·서사 장치 여섯)와 §17(다음 확인)을 게임 화면에 올렸다. **규칙·상수·세이브 버전은 그대로다.**
엔진이 새로 기억하는 것은 선택 필드 둘뿐이다(§18.4).

### 18.1 문구의 자리 — `app/src/game/lore.ts` 하나

| 무엇 | lore.ts | 쓰는 화면 |
| --- | --- | --- |
| 세계 다섯 문장 | `WORLD_LINES` | 팀 등록증 |
| 크루 명찰(이름·맡은 일) | `CREW`, `CREW_ORDER` | 팀 등록증, 모든 `CrewNote` |
| 화면별 한 줄 | `SCREEN.{dig,dispatch,base,appraisal,vault,storage,museum,auction,market,codex,rank}` | 아래 §18.3 |
| 반응 유예 줄 | `TIP_GRACE` | 제보 배너(보이는 줄은 "여섯 팀이 같이 들었다", 전체 문장은 툴팁) |
| 마지막 확인 | `LAST_CHECKED` | 기록 카드, 도감 상세(소장처와 한 줄) |
| 달력 | `worldMonth(t)` — 2351년 5월부터 게임 시각 30일마다 한 달 | 기록 카드, 팀 등록증 |
| 다음 확인 | `nextCheckName`, `teamName`, `CHECKER` | 기록 카드 |
| 산 사람 한 줄 | `buyerLine(artifactId, uid)` — `hashFrac` 결정론 | 매각 결과 카드, 다중 매각 결과 줄, 경매 낙찰 로그 |
| 찾는 한 점 | `WANTED_CANDIDATES`(유일 12), `wantedOf`, `wantedStatus`, `wantedLine` | 팀 등록증, 도감, 원장, 엔딩 |
| 복귀 요약 | `OFFLINE_DID`(매번), `rosterLine(t, away)`(6시간 이상 비운 복귀 중 하루 단위 해시가 ⅓에 걸릴 때만) | 복귀 토스트·모달 |
| 현존 수량 = 공급 | `STOCK_NOTE` | 도감 상세의 세계 재고 줄 |
| 엔딩 | `shownCount`, `KING_TITLE` | 엔딩 배너 |

### 18.2 기록 카드 (`ui/RecordCard.tsx`)

```
┌ 유물명                                 티어 ┐
│ 소장처       데이터의 holder 그대로          │  ← 실재 기록
│ 마지막 확인  2094년 3월                      │
│ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ │
│ 확인자       미라 아냥고                     │  ← 픽션(흑연색)
└ 다음 확인    2351년 5월 · (이름)             ┘
...
다음 확인 칸의 이름: 소장 중이면 `팀 이름 — 우리만 본다`, 전시 중이면 `○○ 박물관(또는 임시 전시대) — 누구나
본다`, 판 직후면 산 사람 한 줄. 카드가 서는 자리는 소장고의 "가장 최근에 연 상자"(아무것도 고르지 않았을 때),
유물 상세, 감정 연출(국보 이상), 매각 결과.

### 18.3 화면별 적용 (§15.3 표의 실제)

| 화면 | 자리 | 누가 |
| --- | --- | --- |
| 온보딩 | 발굴 탭 맨 위 **팀 등록증** 카드(모달 아님, 30분 뒤 기본 접힘) | 전원 |
| 발굴 | 발굴단 카드 머리 | 서가온 |
| 파견 시트 | 머리 | 서가온 |
| 첫 거점 카드 | 제목 아래 | 서가온 |
| 제보 배너 | 반응 유예 줄 | — |
...
| 시설 — 경매장 | 머리 | 정도경 |
| 시장 | 맨 위 | 정도경 |
| 도감 | 머리, 상세의 마지막 확인·세계 재고 줄, 찾는 한 점 표시 | 미라 |
| 순위표 | 목록 위 | 예외7 |
| 복귀 요약 | 고정 4줄 **밖**(4줄은 그대로) | 예외7(장치 ④) |
| 엔딩 | 배너 둘째 줄 | — (장치 ⑥) |
| 규칙(📋) | 변경 없음 | — |

### 18.4 엔진·세이브

| 필드 | 쓰는 함수 | `advance()` 안? | 세이브 |
| --- | --- | :---: | --- |
| `World.wanted?: string` | `setWanted`(유일만 받는다) | 아니다 | v11 그대로. 없으면 시작 거점의 유일 |
| `World.shownSpecies?: string[]` | `displayArtifact` | 아니다 | v11 그대로. 없으면 지금 전시 중인 것. 시즌 롤오버에서 비운다 |

경매 낙찰 로그 문자열 끝에 산 사람 한 줄이 붙는다. 로그 문자열은 계측기가 읽지 않는다.

### 18.5 크루 도트

원본 `ip/src/crew-sprites.mjs` → `node ip/src/export-crew.mjs` → `app/src/render/crew.generated.ts`(생성 파일)
→ `render/crew.ts`가 캔버스로 찍는다. `--check`는 산출물이 원본과 같은지만 본다. 종이색 칸 위에 1배(명찰)·2배
(등록증)로 올린다.

### 18.6 통과 조건과 결과(`eval.md` §39)

1. `density --seeds 12` — 적용 전 출력과 **바이트 단위로 같다**(시간 줄 제외). 14축 후퇴 0.
2. 첫 60초 세계 명제 3개 이상, 첫 10분 크루 3인 이상 — `play lore`가 잰다.
3. 규칙 화면과 상수의 숫자 일치 — `play rules`. lore.ts는 수치를 만들지 않는다.
