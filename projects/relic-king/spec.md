...
**게임의 규격은 이 절 때문에 바뀌지 않는다.** 규칙·상수·세이브·UI 문자열 변경 0이다(G124).
...
**화면에 처음 닿을 자리(제안, 미적용)**: §15.5 장치 ①의 한 줄 아래에 "다음 확인: 2351년 5월 · (기록패 이름)".
적용 조건은 §15.4와 같다(`density --seeds 12` 14축 후퇴 0 · `play` · 규칙 화면 수치 일치).
