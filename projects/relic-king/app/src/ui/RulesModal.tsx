import {
  ...
  TIP_FOCUS_DIG_COST_MULT, TIP_FOCUS_DIG_HIT_CHANCE, TIP_MEAN_INTERVAL, TIP_EMERGENCY_CREW_FUNDS_SHARE, TIP_EMERGENCY_CREW_MIN_COST, TIP_EMERGENCY_CREW_HIT_CHANCE, TIP_RIVAL_FOCUS_HIT,
...
import { clock, usd, withJosa } from "../game/format";
    ...
    <Modal title="규칙" onClose={onClose} wide>
      ...
      <section className="rules-block">
        ...
        <ul className="rules-notes">
          ...
          <li><strong>유예가 끝나고 {TIP_DECIDE_AFTER_GRACE_SECONDS}초 안에 아무도 못 맞히면 그 자리에서 결판낸다.</strong> 승산은 당신 {Math.round(TIP_PLAYER_HIT * 100)}%(집중 굴착이면 {Math.round(TIP_FOCUS_DIG_HIT_CHANCE * 100)}%) 대 <strong>라이벌 머릿수 × {Math.round(TIP_RIVAL_HIT * 100)}%</strong>(국보 이상은 × {Math.round(TIP_RIVAL_FOCUS_HIT * 100)}%)다 — 경쟁자가 1명이면 반반, 3명이면 25%. 배너에 그 판의 승산이 그대로 적힌다. 제보가 결과 없이 지나가는 일은 없다.</li>
          ...
          <li>on_site 팀이 있으면(또는 직접 발굴이 그 자리를 파고 있으면) 자동으로 {Math.round(TIP_PLAYER_HIT * 100)}% 확률로 판정된다 — <strong>같은 제보를 받은 라이벌도 {Math.round(TIP_RIVAL_HIT * 100)}%로 같다. 국보 이상은 라이벌도 집중해서 {Math.round(TIP_RIVAL_FOCUS_HIT * 100)}%로 쫓는다.</strong> 아무것도 안 누르면 공정한 레이스이고, [집중 굴착]을 누르면 {Math.round(TIP_FOCUS_DIG_HIT_CHANCE * 100)}%로 오르는 대신 그 원정의 원정비가 {TIP_FOCUS_DIG_COST_MULT}배가 된다. <strong>진귀·국보 제보는 현지 팀이 있으면 [집중 굴착]이 자동으로 걸리고, 유일만 직접 누른다.</strong> 한 원정의 배수는 곱해지지 않고 가장 큰 것 하나만 붙으며(집중 {TIP_FOCUS_DIG_COST_MULT}배·급파 {EMERGENCY_DISPATCH_COST_MULT}배), 정산액은 보유 자금을 넘지 않는다. <strong>유일 제보에 발굴단이 없고 직접 발굴만 그 자리에 있으면 [긴급 인부]로 대응한다</strong> — 값은 현재 자금의 {Math.round(TIP_EMERGENCY_CREW_FUNDS_SHARE * 100)}%(최소 {usd(TIP_EMERGENCY_CREW_MIN_COST)})이고, 적중은 {Math.round(TIP_EMERGENCY_CREW_HIT_CHANCE * 100)}%다(현지 팀의 [집중 굴착]은 {Math.round(TIP_FOCUS_DIG_HIT_CHANCE * 100)}%).</li>
